...
public class Application01 {
    public static void main(String[] args) {
        ...
        // Mammal 클래스는 Rabbit 부모타입으로 타입이 맞지 않는다.
        // RabbitFarm<Mamml> farm2 = new RabbitFarm<Mamml>();

        // Snake는 Rabbit과 관련이 없는 타입으로 타입이 맞지 않는다.
        // RabbitFarm<Snake> farm3 = new RabbitFarm<Snake>()

        // Rabbit은 Rabbit과 같은 타입으로 제네릭스 타입 지정이 가능하다.
        RabbitFarm<Rabiit> farm4 = new RabbitFarm<>(new Rabiit());
        farm4.getAnimal().cry();

        // Bunny는 Babbit의 자식 타입으로 제네릭스 타입의 지정이 가능하다.
